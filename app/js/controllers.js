...
.controller('topbarCtrl', ['$scope','$rootScope','$state', function($scope,$rootScope,$state) {
    ...
    vm.setting=function(){
        ...
        $state.go("index.myRoom.info.basic");
    }
    vm.defaultIndex=function(){
         $rootScope.any.activeLi=0;
         $state.go("index.myRoom.course");
    }
}])
/********登录操作**********/
...
.controller('major_courseCtrl', ['$scope','$rootScope','MajorCourse','MyCourse', function($scope,$rootScope,MajorCourse,MyCourse) {
    ...
    /*********分类导航函数*********/
    vm.courseNav1 = function(evt, index) {
        vm.activeType = index;
        vm.courType1 = evt.target.innerHTML;
        if (vm.courType1 == '不限')
            vm.courType1 = '';
    };
    vm.courseNav2 = function(evt, index) {
        vm.activeProperty = index;
        vm.courType2 = evt.target.innerHTML;

        if (vm.courType2 == '不限')
            vm.courType2 = '';
    };
    vm.courseNav3 = function(evt, index) {
        vm.activeCredit = index;
        vm.courType3 = evt.target.innerHTML;

        if (vm.courType3 == '不限')
            vm.courType3 = '';
    };
    vm.courseNav4 = function(evt, index) {
        vm.activeTerm = index;
